...
class MathCalculator {
    constructor() {
        ...
        this.builtinFunctions = [
            ...
            'multiply', 'divide', 'pow', 'sinh', 'cosh', 'tanh'
        ];
    ...
    setupMathEnvironment() {
        ...
        // 导入除log之外的所有内置函数
        const allFunctions = Object.keys(math).filter(key => 
            typeof math[key] === 'function' && key !== 'log'
        );
        
        const functionsToImport = {};
        allFunctions.forEach(funcName => {
            functionsToImport[funcName] = math[funcName];
        });
        
        this.math.import(functionsToImport);
        ...
        this.math.import({
            log: (x) => {
                if (x instanceof math.Complex) {
                    // 处理复数情况
                    const result = math.divide(math.ln(x), math.ln(10));
                    return result;
                }
                // 处理实数情况
                return Math.log10(x);
